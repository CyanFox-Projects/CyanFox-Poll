...
Swal.fire({
  title: "Fehler",
  text: "Die maximale Anzahl an Stimmen für ' . $poll_answer_votes['title'] . ' wurde erreicht.",
  icon: "error",
  confirmButtonText: "Ok"
    ...
    window.location = "vote.php?id=' . $_GET['id'] . '";
                            ...
                            $poll_answer_count = $poll_answer_votes['voted_votes'] + 1;

                            $stmt = $pdo->prepare('INSERT INTO `poll_vote`(`poll_id`, `vote`, `name`, `deleteKey`) VALUES ("' . $poll['id'] . '", "' . $value . '", "' . $name . '", "' . $deleteKey . '")');
                            $stmt->execute([$_GET['id']]);

                            $stmt = $pdo->prepare('UPDATE `poll_answers` SET `voted_votes`="' . $poll_answer_count . '" WHERE title = "' . $poll_answer_title . '" AND poll_id = ' . $poll_id . ';');
                            $stmt->execute([$_GET['id']]);
    ...
    <form action="vote.php?id=<?= $_GET['id'] ?>" method="post">
        ...
        <?php for ($i = 0; $i < count($poll_answers); $i++): ?>
            ...
            <label style="word-wrap:break-word">
                ...
                <?= $poll_answers[$i]['title'] ?> (<?= $poll_answer_votes['voted_votes'] ?> von <?= $poll_answer_votes['max_votes'] . " haben schon dafür Abgestimmt" ?>)
            </label>
        <?php endfor; ?>
        <div>
            ...
            <a href="view.php?id=<?= $poll['id'] ?>">Alle Stimmen</a>
        </div>
    </form>
</div>
